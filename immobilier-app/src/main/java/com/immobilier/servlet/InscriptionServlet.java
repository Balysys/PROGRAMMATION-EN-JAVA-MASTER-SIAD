...
public class InscriptionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (!mdp.equals(confirm)) {
            req.setAttribute("erreur", "Mots de passe différents");
            doGet(req, resp);
            return;
        }
        
        if (userDAO.trouverParEmail(email) != null) {
            req.setAttribute("erreur", "Email déjà utilisé");
            doGet(req, resp);
            return;
        }
        
        // Stocker le mot de passe en clair (pour test)
        User user = new User(nom, email, mdp, "utilisateur");
        
        if (userDAO.inscrire(user)) {
            resp.sendRedirect(req.getContextPath() + "/connexion?inscription=ok");
        } else {
            req.setAttribute("erreur", "Erreur technique");
            doGet(req, resp);
        }
    }
}
