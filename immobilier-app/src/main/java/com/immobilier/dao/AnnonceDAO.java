...
public class AnnonceDAO {
    ...
    public List<Annonce> toutesLesAnnonces() {
        ...
        // Requête modifiée pour récupérer aussi le nom de l'utilisateur
        String sql = "SELECT a.*, u.nom as proprietaire_nom FROM annonces a JOIN users u ON a.utilisateur_id = u.id ORDER BY a.id DESC";
        try (Connection conn = DBUtil.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql); ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                ...
                a.setProprietaireNom(rs.getString("proprietaire_nom"));
                list.add(a);
            }
    ...
    public Annonce trouverParId(int id) {
        String sql = "SELECT a.*, u.nom as proprietaire_nom FROM annonces a JOIN users u ON a.utilisateur_id = u.id WHERE a.id = ?";
        try (Connection conn = DBUtil.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, id);
